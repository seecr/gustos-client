...
down_re = re.compile(
    r"/etc/service/lxc-(?P<vm>[^:].*): down (?P<seconds>[0-9]+) seconds$"
)
down_up_re = re.compile(
    r"/etc/service/lxc-(?P<vm>[^:].*): down (?P<seconds>[0-9]+) seconds, normally up$"
)
up_re = re.compile(
    r"/etc/service/lxc-(?P<vm>[^:].*): up \(pid [0-9]+\) (?P<seconds>[0-9]+) seconds"
)
...
def svstat():
    result = subprocess.run(
        "svstat /etc/service/lxc-*", capture_output=True, text=True, shell=True
    )
